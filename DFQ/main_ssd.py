...
parser.add_argument("--dataset", type=str, default='/home/xwl/Project/data/VOCdevkit/VOC2007/', help="The root directory of the VOC dataset or Open Images dataset.")
...
if __name__ == '__main__':
    ...
    desktop_path = "/home/xwl/Project/DFQ/"  # 新创建的txt文件的存放路径
    full_path = desktop_path + 'weights.txt'  # 也可以创建一个.doc的word文档
    ...
    for name, parameters in net.state_dict().items():
        if "weight" in name:
            file.write(name)
            file.write('\r\n')
            file.write(str(parameters.detach().cpu().numpy()))
            file.write('\r\n--------------------------------------------------------------------\r\n')
    file.close()
    ...
    if args.log:
        with open("ssd_result.txt", 'a+') as ww:
            ...
            ww.write("{}, quant: {}, relu: {}, equalize: {}, absorption: {}, correction: {}, clip: {}\n".format(
                args.dataset_type, args.quantize, args.relu, args.equalize, args.absorption, args.correction, args.clip_weight
            ))
