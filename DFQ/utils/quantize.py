...
class UniformQuantize(InplaceFunction):
    ...
    将权重量化为[qmin,qmax]即[-128,127]或[0,255]之间的整数
    ...
    def forward(ctx, input, num_bits=8, min_value=None, max_value=None, inplace=False, symmetric=False, num_chunks=None):
        ...
        if min_value is None or max_value is None:
            B = input.shape[0]
            y = input.view(B // num_chunks, -1)  #reshape 1xn
...
class QuantMeasure(nn.Module):
    ...
    def forward(self, input):
        if self.update_stat:
            ...
            self.running_max = max(self.running_max, input.detach().contiguous().view(input.size(0), -1).max(-1)[0].mean())
            self.running_min = min(self.running_min, input.detach().contiguous().view(input.size(0), -1).min(-1)[0].mean())
