...
import models
from clr import CyclicLR  # 周期性学习策略，以恒定的频率在两个边界之间循环学习率。两个边界之间的距离可以在每个迭代或每个周期的基础上进行缩放。周期性学习率策略会在每批之后更改学习率。
...
def main():
    ...
    for epoch in trange(args.start_epoch, args.epochs + 1):
        ...
        csv_logger.write_text('\n\nEpoch {}: Best accuracy is {:.2f}% top-1'.format(epoch + 1, best_prec1 * 100.))
